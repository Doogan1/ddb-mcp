...
export async function deleteCharacter(
  ...
  if (params.confirm !== true) {
    return {
      content: [
        {
          type: "text",
          text: "Deletion aborted. Pass confirm=true and the exact character name to permanently delete a character.",
        },
      ],
    };
  }

  const expectedName = params.characterName?.trim();
  if (!expectedName) {
    return {
      content: [
        {
          type: "text",
          text: "Deletion aborted. characterName is required and must match the character exactly.",
        },
      ],
    };
  }
  ...
  if (character.name.trim().toLowerCase() !== expectedName.toLowerCase()) {
    return {
      content: [
        {
          type: "text",
          text: `Deletion aborted. Character ${params.characterId} is named "${character.name}", which does not match "${params.characterName}".`,
        },
      ],
    };
  }
  ...
  return {
    content: [
      {
        type: "text",
        text: `Deleted character ${character.name} (${params.characterId}).`,
      },
    ],
  };
}
