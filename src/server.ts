...
import { checkAuth } from "./tools/auth.js";
import {
  getCharacter,
  getDefinition,
  listCharacters,
...
import { modeAllows, parseToolMode, type ToolMode } from "./config/tool-mode.js";
...
export function createMcpServer(options: CreateMcpServerOptions = {}): {
  ...
  const mode = options.mode ?? parseToolMode(process.env.DDB_MCP_MODE);
  ...
  server.tool(
    "check_auth",
    "Check authentication status and verify if your D&D Beyond session is valid. Login is a CLI command (`npm run setup`), not an agent tool.",
    {},
    READ_ONLY,
  ...
  if (modeAllows(mode, "builder")) {
    ...
    server.tool(
      "delete_character",
      "Permanently delete a character from D&D Beyond. Requires confirm=true and the exact character name. This cannot be undone.",
      {
        characterId: z.coerce.number().describe("The character ID to delete"),
        characterName: z.string().describe("Exact character name; must match the character being deleted"),
  ...
  return { server, client, mode };
}

export async function startServer(): Promise<void> {
  const { server, mode } = createMcpServer();

  const transport = new StdioServerTransport();
  await server.connect(transport);

  const shutdown = async () => {
    console.error("ddb-mcp: shutting down...");
    await server.close();
    process.exit(0);
  };
  ...
  console.error(`ddb-mcp: server running (mode=${mode})`);
}
